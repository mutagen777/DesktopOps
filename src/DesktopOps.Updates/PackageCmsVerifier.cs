...
/// <summary>Verifies detached CMS/PKCS#7 signatures for downloaded packages.</summary>
public static class PackageCmsVerifier
{
    ...
    /// optionally requiring the signer thumbprint to be in <paramref name="trustedThumbprints"/>.
    /// </summary>
    ...
    public static HashSet<string> ParseTrustedThumbprints(string? configured)
    {
        ...
        {
            var normalized = NormalizeThumbprint(part);
        ...
        {
            throw new InvalidOperationException(
                "TrustedCmsThumbprints is configured but contains no valid thumbprints.");
        }
